package com.example.user.sports.ui.decoration;
...
import com.example.user.sports.utils.bean.BaseIndexTagBean;
...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_BG = Color.parseColor("#00000000");
    private static int COLOR_TITLE_FONT = Color.parseColor("#FFffffff");
    private static int mTitleFontSize;//title字体大小


    public TitleItemDecoration(Context context, List<? extends Friend> datas) {
        super();
        mDatas = datas;
        mPaint = new Paint();
        mBounds = new Rect();
...
/*
        ...
        c.drawText(mDatas.get(position).getTag(), 20, child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2), mPaint);
    }
...
//        View child = parent.findViewHolderForPosition(pos).itemView;//出现一个奇怪的bug，有时候child为空，所以将 child = parent.getChildAt(i)。-》 parent.findViewHolderForLayoutPosition(pos).itemView
//
